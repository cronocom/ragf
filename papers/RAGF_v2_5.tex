...
Evaluation across 12{,}847 aviation actions in an operational simulation and 1{,}893 healthcare actions in a production-parallel (shadow) environment demonstrates feasibility: no action was executed against live aircraft or live patients. Within this scope, 41 unsafe actions were flagged for denial with zero false positives within the validated operational scope, sub-30ms governance latency at p95, and fail-closed behavior across seven enumerated failure categories validated through 3{,}500 systematic failure injections.
...
\begin{enumerate}
    ...
    \item \textbf{Governance Infrastructure:} Implementation of Validation Gate, semantic ontology integration, and cryptographic audit mechanisms, evaluated across aviation and healthcare settings.

    \item \textbf{Empirical Safety Evidence:} Demonstration of fail-closed enforcement across 14{,}740 actions evaluated in an operational simulation (aviation) and a production-parallel shadow environment (healthcare), together with 3{,}500 injected failures, with explicit formalization of guarantee scope and limitations.
...
RAGF implements boundary enforcement through four interoperating components: the Validation Gate (deterministic enforcement), Semantic Authority Layer (domain ontologies), Cryptographic Audit (tamper-evidence under an honest-key assumption), and Escalation Pathways (human override).
...
\paragraph{Tamper-Evidence, Not Non-Repudiation} We claim tamper-evidence under an honest-key assumption, not non-repudiation. HMAC-SHA256 is a symmetric message authentication code: any party holding the shared key can forge or alter ledger entries, consistent with the key-compromise case acknowledged in our threat model (Section~\ref{sec:threat-model}). The audit layer therefore detects tampering by parties without the key, but does not cryptographically bind a verdict to a specific signer. Non-repudiation would require an asymmetric signature scheme (e.g., per-authority signing keys), which we identify as a direct extension for deployments that must attribute verdicts to individual authorities.
...
\paragraph{Ethics and Data Governance} The healthcare evaluation operated in a production-parallel shadow mode and required no ethics-board (IRB) review: no recommendation reached a patient, no clinical order was issued, and the evaluation acted on no identifiable patient data. The HL7 FHIR interface was exercised against fully anonymized records containing no protected health information and no real clinical decisions, so the evaluation presented no risk to patients and no use of identifiable data. The data-quality artifacts we report (e.g., occasional stale laboratory values, Section~\ref{sec:state-complexity}) arise from the anonymized test data and the eventual-consistency behavior of the FHIR integration; they are integration-engineering observations, not clinical events. We emphasize that this absence of an IRB requirement is specific to a non-identifiable, shadow-mode feasibility evaluation: any live deployment acting on identifiable patient data and influencing care would require IRB approval, a lawful basis for data use, and a formal data-governance arrangement, which we identify as prerequisites for clinical use.
...
\paragraph{Results} Across 1,787 total denials (1,544 aviation plus 243 healthcare), reviewers classified all as true positives, leaving the false-positive category empty. Eleven cases across both domains requiring regulatory or clinical interpretation were resolved through discussion and reference to advisory circulars or clinical guidelines.

\paragraph{Interpretation of the Zero False-Positive Result} Because every reviewed denial was labeled a true positive, the classification task is degenerate: one of the two categories has no instances. We therefore do not report Cohen's $\kappa$ as a measure of agreement over a balanced two-category task, since $\kappa$ is undefined (or uninformative) without variance in the labels being compared. What the dual review establishes is the weaker but well-supported claim that \emph{both reviewers independently judged no denial to be a false positive}. We further caution that a $12\%$ denial rate with zero false positives across both domains is consistent with the encoded rules being bright-line, low-ambiguity constraints (e.g., hard regulatory thresholds and formulary boundaries) rather than evidence that the gate resolves contested or interpretive cases correctly. We do not claim the zero false-positive result generalizes to rule sets that require contested interpretation.
...
\paragraph{Interpretation} This comparison is not strictly like-for-like. Human reviewers are slower in part because they resolve ambiguity that the ontology cannot encode---precisely the 0.8--2.0\% of cases that RAGF escalates to them. The throughput gain therefore applies only to the subset of actions reducible to deterministic predicates; it should not be read as RAGF outperforming human judgment on the residual, interpretation-heavy cases. Redeployed domain experts are not eliminated but reallocated: in our settings they shift from per-action review toward escalation adjudication, ontology and rule maintenance, and exception handling---work that itself constitutes a recurring operational cost (Section~\ref{sec:operational-sustainability}).
...
\label{sec:operational-sustainability}
...
and jurisprudence growth patterns, and to estimate inter-operator consistency
under literature-based assumptions (Section~\ref{sec:inter-operator}).
...
\subsubsection{Estimated Inter-Operator Consistency (Literature-Based)}
\label{sec:inter-operator}

We did not perform systematic multi-operator review during this feasibility
evaluation. To provide an \emph{a priori} estimate of whether the ontology
offers sufficient guidance for consistent human judgment, we modeled
inter-operator agreement using experience-stratified disagreement
distributions drawn from aviation human-factors and clinical
decision-making literature~\cite{kahneman2009conditions}, applied to the
$n=100$ aviation and $n=38$ healthcare escalation cases.

\paragraph{Estimation, not measurement} Under these literature-derived
assumptions, the model yields an estimated mean agreement of approximately
$95\%$ in aviation and $94\%$ in healthcare, with simulated pairwise ranges
of roughly $94$--$97\%$ (aviation) and $92$--$97\%$ (healthcare). We stress
that these are \textbf{modeled estimates, not observed agreement rates}:
no systematic multi-operator review was conducted, and the operator
distributions are taken from prior literature rather than measured in our
deployments. The figures should be read as a design-stage expectation,
comparable to inter-rater reliability of Cohen's $\kappa \approx 0.85$--$0.90$
reported in expert-judgment studies~\cite{kahneman2009conditions}, and
require empirical confirmation through instrumented multi-operator review
in future work.

\paragraph{Interpretation} If borne out empirically, agreement at this level
would suggest that the ontology and validation rules constrain operator
discretion enough to support consistent resolution of edge cases, with
residual disagreement concentrated on boundary cases where regulatory
thresholds admit legitimate interpretive variance. We do not claim this
outcome as a result of the present study.
...
\begin{itemize}
\item Aviation: 38\% of unique escalations generated new rules (38/100 cases)
\item Healthcare: 39\% of unique escalations generated new rules (15/38 cases)
\end{itemize}
...
In a full production deployment, we would expect rule creation rates to decay as the ontology
...
studies~\cite{kahneman2009conditions}, as systematic multi-operator review was not
...
Closest to RAGF's Validation Gate is the line of work on \emph{shielding} for safe reinforcement learning, where a deterministic shield filters or corrects agent actions to preserve formal safety properties. Recent advances extend shielding to partially observable settings with deep RL integration \cite{carr2023shielding}, optimize task reward while preserving provable safety through local planning \cite{rahmani2024dmps}, and provide probabilistic guarantees via state-augmented MDPs \cite{hameldelacourt2025probabilistic}. RAGF's gate can be read as a deterministic shield specialized to regulated domains; our contribution is therefore not a new shielding algorithm but a relocation of the certification target---from the (uncertifiable) reasoning model to the deterministic enforcement harness---together with the domain-ontology grounding and audit infrastructure that regulated deployment requires. Unlike shields designed to preserve an RL agent's reward objective, RAGF's enforcement is reward-agnostic and answers only to encoded regulatory constraints.
...
RAGF demonstrates that deterministic boundary enforcement can mediate agentic execution in regulated environments, achieving operational viability and fail-closed behavior within tested scope. Across 14,740 actions evaluated in an operational simulation (aviation) and a production-parallel shadow environment (healthcare), RAGF flagged 41 unsafe actions for denial while maintaining sub-30ms governance latency at p95.
...
\bibitem{carr2023shielding}
Steven Carr, Nils Jansen, Sebastian Junges, and Ufuk Topcu.
\newblock Safe Reinforcement Learning via Shielding under Partial Observability.
\newblock In {\em Proceedings of the AAAI Conference on Artificial Intelligence}, volume~37, pages 14748--14756, 2023.
...
\bibitem{hameldelacourt2025probabilistic}
Edwin Hamel-De le Court, Francesco Belardinelli, and Aaron~W. Goodall.
\newblock Probabilistic Shielding for Safe Reinforcement Learning.
\newblock In {\em Proceedings of the AAAI Conference on Artificial Intelligence}, volume~39, pages 16091--16099, 2025.
...
\bibitem{rahmani2024dmps}
Kasra Rahmani et al.
\newblock Dynamic Model Predictive Shielding for Provably Safe Reinforcement Learning.
\newblock In {\em Advances in Neural Information Processing Systems 37 (NeurIPS)}, 2024.
